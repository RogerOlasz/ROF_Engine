...
void PanelComponents::DrawMaterial(ComponentMaterial* go_material, GameObject* go_selected)
{
	if (ImGui::CollapsingHeader(go_material->name.c_str()))
	{
		...
		ImGui::Separator();
		ImGui::Image((ImTextureID*)go_material->GetTextureId(), ImVec2(200, 200), ImVec2(0, 0), ImVec2(1, 1), ImVec4(1, 1, 1, 1), ImVec4(0.0f, 0.6f, 0.6f, 1.0f));
		ImGui::Text("%s%s", "Texture path: ", go_material->GetTexturePath());
